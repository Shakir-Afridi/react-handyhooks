---
# https://vitepress.dev/reference/default-theme-home-page
layout: home

hero:
  name: "React Hookstack"
  tagline: A lightweight and powerful collection of reusable React hooks designed to simplify state management, event handling, and UI logic in your React apps.
  actions:
    - theme: brand
      text: Documentation
      link: /state/useArray

features:
  - title: Smart utilities
    details: Abstract common React logic into reusable hooks
  - title: Lightweight
    details: Fast, efficient, and easy to integrate
  - title: TypeScript support 
    details:  Fully typed API
  - title: Composable
    details: Integrate easily into existing code
  - title: Framework agnostic 
    details:  Works with any React setup (Vite, CRA, Next.js, etc.)
---

## 🚀 Features

- 🧠 **Smart utilities** — abstract common React logic into reusable hooks  
- 🪶 **Lightweight** — fast, efficient, and easy to integrate
- ⚙️ **TypeScript support** — fully typed API  
- 🧩 **Composable** — integrate easily into existing code  
- 🔧 **Framework agnostic** — works with any React setup (Vite, CRA, Next.js, etc.)

## 📘 Storybook

Explore all hooks interactively on Storybook:  
👉 [Live Demo](https://shakir-afridi.github.io/react-handyhooks/storybook)

## 👨‍💻 Author

React Hookstack — maintained by passionate open-source developers who believe in clean, reusable React logic

## ⭐ Support

If you find this library useful, please give it a star ⭐ on [GitHub](https://github.com/Shakir-Afridi/react-handyhooks) —
it helps others discover and support the project!